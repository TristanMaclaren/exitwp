...
'''
exitwp - Wordpress xml exports to Jekyll blog format conversion
...
def get_namespaces(xml_file):
    """Extract namespaces from an XML file."""
    namespaces = {}
    for event, elem in ET.iterparse(xml_file, events=('start-ns',)):
        prefix, uri = elem
        namespaces[prefix] = f'{{{uri}}}'
    return namespaces
...
def parse_wp_xml(file):
    namespaces = get_namespaces(file)
    tree = ET.parse(file)
    root = tree.getroot()
    ns = namespaces.copy()
    ns[''] = ''
        ...
        for i in xml_items:
            ...
            def gi(q, unicode_wrap=True, empty=False):
                ...
                if ':' in q:
                    ...
                    tag = q
                try:
                    result = (i.find(q, ns) or i.find(tag) or i.find(ns.get(namespace, '') + tag)).text.strip()
...
def write_jekyll(data, target_format):
    ...
    def get_item_uid(item, date_prefix=False, namespace=''):
        ...
        if item['wp_id'] in item_uids[namespace]:
            ...
            while fn in item_uids[namespace]:
                n += 1
                fn = ''.join(uid) + '_' + str(n)
            item_uids[namespace][item['wp_id']] = fn
            result = fn
        return result
    ...
    def get_attachment_path(src, dir, dir_prefix='images'):
        ...
        try:
            ...
            while maybe_filename in current_files:
                maybe_filename = f"{file_root}-{file_infix}{file_ext}"
                file_infix += 1
    ...
    for i in data['items']:
        ...
        out = None
        try:
            date = datetime.strptime(i['date'], date_fmt).replace(tzinfo=UTC())
        except:
            date = datetime.today()
        ...
        if i['type'] == 'post':
            ...
            item = i
            while item['parent'] != '0':
                parent = next((parent for parent in data['items']
                               if parent['wp_id'] == item['parent']), None)
                if parent:
                    parentpath = get_item_uid(parent) + '/' + parentpath
                    item = parent
                else:
                    break
...
wp_exports = glob(os.path.join(wp_exports, '*.xml'))
